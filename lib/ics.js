...
function buildEvent(attributes) {
  if (!attributes || _.isEmpty(attributes)) {
    ...
    return _.compact([
      'BEGIN:VCALENDAR',
      'PRODID:' + attributes.prodid || '-//Adam Gibbons//agibbons.com//ICS: iCalendar Generator',
      'VERSION:2.0',
      'CALSCALE:GREGORIAN',
      'METHOD:REQUEST',
    ]
    ...
    .concat([
      ...
      'CREATED:' + generateDateTimeStamp(),
      'LAST-MODIFIED:' + generateDateTimeStamp(),
  ...
  function formatDTEND(startString, endString, tz, tzEnd) {
    ...
    if (endString && isDateTime(endString)) {
  ...
  function formatAttendees(attributes) {
    if (attributes.attendees) {
      return attributes.attendees.map(function (attendee) {
        ...
        if (attendee.rsvp) {
          parts.push('RSVP=' + attendee.rsvp);
        }
        if (attendee.name && attendee.email) {
          var guestsCount = _.isFinite(attendee.guests) ? attendee.guests : 0;
          var str = 'CN=' + attendee.name + ';';
          str += 'X-NUM-GUESTS=' + guestsCount + ':';
          str += 'mailto:' + attendee.email;
          parts.push(str);
        }
        return parts.length ? ['ATTENDEE'].concat(parts).join(';') : null;
      });
    }
